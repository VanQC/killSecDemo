...
func SelectCountByGoodsIdPcc(gid int) (int64, error) {
	...
	sql := `select * from promotion_sec_kills where goods_id = ? for update`
	err := DB.Raw(sql, gid).First(&skGood).Error
...
func ReduceByGoodsId(gid int) (int64, error) {
	var count int64
	sqlStr := `UPDATE promotion_sec_kills SET ps_count = ps_count-1 WHERE ps_count>0 AND goods_id = ?`
	res := DB.Exec(sqlStr, gid)
	if err := res.Error; err != nil {
		return count, err
	}
	count = res.RowsAffected // 获取受影响行数
	return count, nil
}

// 减少一个
func ReduceOneByGoodsId(gid int) error {
	sqlStr := `UPDATE promotion_sec_kills SET ps_count = ps_count-1 WHERE goods_id = ?`
	res := DB.Exec(sqlStr, gid)
	return res.Error
}
...
func ReduceStockByOcc(gid int, num int, version int) (int64, error) {
	var count int64
	sqlStr := "UPDATE promotion_sec_kills SET ps_count = ps_count-?, version = version+1 " +
